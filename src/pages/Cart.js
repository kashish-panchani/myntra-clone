...
const Cart = () => {
  const { removeFromCart, setSelectedItemCount, selectedItemCount } = useCart();
  ...
  const handleItemCheckboxChange = (productId) => {
    const updatedCartItems = cartItems.map((item) => {
      ...
      return item.id === productId ? { ...item, checked: !item.checked } : item;
    });
    setCartItems(updatedCartItems);
    const allItemsSelected = updatedCartItems.every((item) => item.checked);
    console.log("updatedCartItems", updatedCartItems, cartItems);
